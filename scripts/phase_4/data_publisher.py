...
# Dimensiones
# ---------------------
def get_or_create_directory(cur_olap, full_path, dir_cache, olap_conn):
    """
    Crea directorios de forma recursiva asegurando que el padre exista.
    """
    if not full_path or full_path == "." or full_path == "/":
        return None
        
    full_path = str(Path(full_path).absolute())
    
    if full_path in dir_cache:
        return dir_cache[full_path]

    # 1. Intentar buscarlo primero
    ...
    # 2. Si no existe, obtener el ID del padre (recursividad)
    parent_path = str(Path(full_path).parent)
    # Evitar recursión infinita en la raíz
    if parent_path == full_path:
        parent_id = None
    else:
        parent_id = get_or_create_directory(cur_olap, parent_path, dir_cache, olap_conn)

    # 3. Insertar el directorio actual
    try:
        # Usamos un SAVEPOINT para que si este insert falla, no invalide toda la transacción del ETL
        cur_olap.execute("SAVEPOINT dir_insert_savepoint")
        ...
        cur_olap.execute("""
            ...
            ON CONFLICT (full_path) DO UPDATE SET directory_name = EXCLUDED.directory_name
            RETURNING id
        """, (Path(full_path).name, full_path, parent_id, depth, *levels))
        
        dir_id = cur_olap.fetchone()[0]
        cur_olap.execute("RELEASE SAVEPOINT dir_insert_savepoint")
        
    except Exception:
        # Si algo falla (ej. colisión de hilos), volvemos al punto anterior y buscamos el ID
        cur_olap.execute("ROLLBACK TO SAVEPOINT dir_insert_savepoint")
        cur_olap.execute("SELECT id FROM dim_directory WHERE full_path=%s", (full_path,))
        row = cur_olap.fetchone()
...
def get_or_create_dim(cur_olap, table, col, value, default_value, cache, olap_conn):
    ...
    # Si no existe, insertar y HACER COMMIT DE INMEDIATO
    try:
        cur_olap.execute(f"INSERT INTO {table} ({col}) VALUES (%s) ON CONFLICT ({col}) DO UPDATE SET {col}=EXCLUDED.{col} RETURNING id", (val,))
        dim_id = cur_olap.fetchone()[0]
        olap_conn.commit() # Esto asegura que la dimensión exista para fact_files
        cache[val] = dim_id
        return dim_id
    except Exception:
        olap_conn.rollback()
...
# ETL
# ---------------------
def process_files(fact_batch_size=1000, excerpt_batch_size=1000):
    ...
    oltp_count_cur.execute("SELECT COUNT(*) FROM files WHERE hash_pending=false")
    total_files = oltp_count_cur.fetchone()[0]
    pbar = tqdm(total=total_files, desc="ETL files", mininterval=3,maxinterval=30)

    rows = oltp_cur.fetchmany(1000)
    while rows:
        for row in rows:
            try:
                ...
                directory_id = get_or_create_directory(olap_cur, full_path, dir_cache, olap)
                filetype_id = get_or_create_dim(olap_cur, "dim_filetype", "file_type", file_type, "tipo desconocido", type_cache, olap)
                classification_id = get_or_create_dim(olap_cur, "dim_classification", "categoria", categoria, "sin clasificación", class_cache, olap)
                ...
                fact_tuple = (
                    file_id, file_name, directory_id, filetype_id, classification_id, canonical_group_id,
                ...
                if len(fact_inserts) >= fact_batch_size:
                    execute_values(olap_cur, """
                        INSERT INTO fact_files (
                            file_id, name, directory_id, filetype_id, classification_id, canonical_group_id,
